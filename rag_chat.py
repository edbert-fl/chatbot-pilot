...
class WorkingRAGChatBot:
   ...
   content: {chunk['content'][:500]}{'...' if len(chunk['content']) > 500 else ''}"""
    ...
    def create_prompt(self, query: str, context: str) -> str:
        """Create the prompt for the LLM with context and instructions."""
        return f"""You are a helpful banking assistant. Answer the user's question using ONLY the information provided in the context below. 
...
Answer:"""
    ...
    def chat(self, query: str, max_context_chunks: int = 5) -> Dict[str, Any]:
        """Process a query using RAG + OpenAI."""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a helpful banking assistant that provides accurate, cited answers based on provided documents."},
